...
if [ "$1" = "$part2" ]; then
	...
	/sbin/iptables -A FORWARD -i "$client_net" -o "$server_net" -d 192.168.101.2 -p tcp --dport 80 -m hashlimit --hashlimit-name LIMIT --hashlimit-burst 1 --hashlimit-mode srcip,dstport --hashlimit-upto 1/sec -j LOG --log-prefix "****DROPPED PACKETS****" --log-level 4
	...
	# log all the connection that are going to be dropped 1/second being generated
	# with each unique source ip and destination port
	/sbin/iptables -A FORWARD -i "$client_net" -o "$server_net" -d 192.168.101.2 -m hashlimit --hashlimit-name LIMIT --hashlimit-burst 1 --hashlimit-mode srcip,dstport --hashlimit-upto 1/sec -j LOG --log-prefix "****DROPPED PACKETS****" --log-level 4
